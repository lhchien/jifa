...
jQuery(window).load(function() { 
	...
	$('#menu_mb_ll').removeAttr('style');
		...
		$('.new-cart').insertAfter('#menu_mb_ll');
...
$(window).load(function() {
    	...
    	$('.stuck_container:not(.isStuck) #cart').on('click','.heading', function() {
			...
			$('.stuck_container:not(.isStuck) #cart .contentCart').slideToggle("10", function(){if(h> 200){
					$(".stuck_container:not(.isStuck) #boxscroll").niceScroll({cursorcolor:"#2c3628",cursorwidth:8});
			}});
    	});
...
$(document).ready(function () {
	...
	$("#cart, .stuck_container.isStuck #cart").click(function () {		
		if(cc == 1){
			...
			if($(window).width() <= 420){  
				$(".mm-page").animate({height: '1vh'}, 400);
			}else{ 
			...
			if($(window).width() <= 420){  
				$(".mm-page").animate({height: '100%'}, 400);
			}else{ 
	...
	$(".bg-cart, .close-cart").on('click', function () {
		...
		$(".mm-slideout").animate({height: '100%'}, 400);
		$("body").css('overflow-y', 'scroll');
		cc = 1;
	});
...
$(function() {
	...
	$('.multi-level li').hover(function(){
		...
		var minH = parentUL.find('>li').map(function(){ return $(this).height();}).toArray().reduce(function(a, b){ return a + b;});
		minH -= 5;
		
		currentUL.addClass('active')
		currentUL.css({'min-height': minH + 'px'});
		$(this).addClass('active');		 
	})
